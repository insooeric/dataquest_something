"""
WoundScope v3 – ViT-Small + location embedding.

Improvements:
  - ViT-Small backbone (better generalisation on medical images)
  - Location embedding (multimodal)
  - WeightedRandomSampler (fixes class imbalance)
  - MixUp + CutMix augmentation
  - AdamW + weight decay + label smoothing
...
from data_loader import WoundDataset, WOUND_CLASSES, BODY_LOCATIONS, NUM_LOCATIONS, VAL_TRANSFORM
from utils import evaluate, print_report, save_checkpoint, get_device, plot_training_curves


# ──────────────────────────────────────────────────────────────────────────────
# Augmentation (stronger than baseline — helps ViT generalise)
# ──────────────────────────────────────────────────────────────────────────────
...
class WoundScopeV3(nn.Module):
    """ViT-Small backbone + location embedding → 4-class classifier."""

    def __init__(self, arch="vit_small_patch16_224", num_classes=4,
                 num_locations=NUM_LOCATIONS, loc_emb_dim=32,
...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for imgs, locs, labels in loader:
        ...
        if np.random.rand() > 0.5:
            ...
            loss = mix_loss(criterion, logits, la, lb, lam)
...
def main(args):
    ...
    model = WoundScopeV3(arch=args.arch, num_classes=len(WOUND_CLASSES)).to(device)
    print(f"Backbone: {args.arch}  |  Feature dim: {model.feat_dim}")

    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)

    # Phase 1: warm up classifier head only (3 epochs)
    ...
    print(f"\n--- Phase 2: Full fine-tune ({args.epochs} epochs, early stop patience={args.patience}) ---")
    ...
    for ep in range(args.epochs):
        ...
        print(f"Epoch {ep+1:3d}/{args.epochs}  train_loss={tl:.4f} train_acc={ta:.3f}  "
        ...
        if va > best_val_acc:
            ...
            no_improve = 0
            save_checkpoint(
                {"model_state": model.state_dict(), "arch": args.arch, "epoch": ep + 1, "version": "v3"},
    ...
    print_report(preds, labels, out_path="outputs/eval_v3.txt")
